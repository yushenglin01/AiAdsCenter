...
func TestLoadPromptsReadsVersionedContracts(t *testing.T) {
	promptDir := t.TempDir()
	schemaDir := t.TempDir()
	require.NoError(t, os.WriteFile(filepath.Join(promptDir, "business_agent_system_v1.1.0.txt"), []byte("system"), 0o600))
	require.NoError(t, os.WriteFile(filepath.Join(promptDir, "business_agent_user_v1.1.0.txt"), []byte("user"), 0o600))
	...
	require.Equal(t, "1.1.0", prompts.Version)
